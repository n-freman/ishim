...
def home(request):
    ...
    return render(request, f'tm_main/home{suffix}.html', context)


@login_forbidden
def reg_type(request):
    return render(request, 'tm_main/reg_type.html')
...
def login_view(request):
    if request.method == 'POST':
        ...
        if not user:
            ...
            return render(request, 'tm_main/login.html')
        login(request, user)
...
def activate_user(request, uidb64, token):
    ...
    if user and generate_token.check_token(user, token):
        ...
        return redirect(reverse('login-tm'))
    return render(request, 'tm_main/activation_failed.html', {"user": user})
...
def logo_edit(request):
    ...
    if request.POST:
        ...
        print(form.errors)
        return redirect(reverse('profile-tm'))
    return render(request, 'tm_main/edit_logo.html')
...
def profile(request):
    ...
    return render(request, f'tm_main/{prefix}_profile.html')
